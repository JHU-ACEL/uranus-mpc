...
class QPController(Controller):
  ...
  def __init__(self, propagator, Q, Qf, R, state_limits, control_limits, horizon):
    ...
    # Check if attitude Jacobian is square (whether using mrp's or not)
  ...
  def __call__(self, x0: jax.Array, x_goal: jax.Array, key: jax.Array, external_dynamics_params: jax.Array, nominal_traj: jax.Array, nominal_cntrl: jax.Array, Q: Optional[jax.Array] = jnp.eye(7), Qf: Optional[jax.Array] = jnp.eye(7), R: Optional[jax.Array] = jnp.eye(3)) -> Tuple[jax.Array, jax.Array]:
    ...
    ############# JAX-based Moreau solver
    dx0 = self.dynamics.get_error_coords(x0, nominal_traj[0])
    x_goal = jax.vmap(self.dynamics.get_error_coords,in_axes=(None, 0))(x_goal, nominal_traj)
    
    P_data, A_data, q, b = self.form_ocp_moreau(dx0, x_goal, nominal_traj, nominal_cntrl, external_dynamics_params, self.Q, self.Qf, self.R)
    solution = self.solver.solve(P_data, A_data, q, b)
    reshaped = solution.x[nx:].reshape(N, nx + nu)
    info = self.solver.info
    # jax.debug.print("Status: {status}", status=info.status)
    # jax.debug.print("Objective value: {x}", x=info.obj_val)
    # View https://docs.moreau.so/guide/basic-usage.html for status info
    ...
    cntrl_traj = du + nominal_cntrl

    state_traj = jnp.nan_to_num(state_traj)
    cntrl_traj = jnp.nan_to_num(cntrl_traj)

    # Generate nonlinear version for nominal traj for next run
    ...
    #                             num_steps = self.horizon,
    #                             external_dynamics_params = external_dynamics_params,
    #                             control_sequence = cntrl_traj)
    
    
    return state_traj, cntrl_traj